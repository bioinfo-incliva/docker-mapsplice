	...
	if juncdb_index == "":
		...
		if input_reads_2 != "":
		    splice_cmd = [bin_dir + "mapsplice_multi_thread",
		                  ...
		                  "--max_append_mis", str( 3),
		                  "--max_intron_double", str(300000),
...
def DNA2StdRegion(drang_input, std_range_out, extend_len, log_file):
    
    print >> sys.stderr, "[%s] Convert DNA fusion to strandard region format" % (right_now())
    
    mapsplice_log = open(log_file, "w")
    #stdout=mapsplice_log
    
    if os.path.exists(std_range_out) and \
	   rerun_all == 0:
	return std_range_out

    resortmapreads_cmd = [bin_dir + "DNA2StdRegion", 
                          drang_input,
                          std_range_out,
                          str(extend_len)]
    ...
    try:    
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Convert DNA fusion to strandard region format failed"
            exit(1)
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: DNA2StdRegion not found on this system"
        exit(1)


def generate_combined_sequence(std_range_input, 
                               synthetic_chromosome_dir, 
                               chromosome_files_dir, 
                               combined_seq_ext,
                               log_file):
    
    print >> sys.stderr, "[%s] generate synthetic chromosome sequence " % (right_now())
    
    mapsplice_log = open(log_file, "w")
    #stdout=mapsplice_log
    
    if os.path.exists(synthetic_chromosome_dir) and \
	   rerun_all == 0:
	return synthetic_chromosome_dir

    resortmapreads_cmd = [bin_dir + "generate_combined_sequence", 
                          std_range_input,
                          synthetic_chromosome_dir,
                          chromosome_files_dir,
                          str(combined_seq_ext)]
    ...
    try:    
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: generate synthetic chromosome sequence failed"
            exit(1)
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: generate_combined_sequence not found on this system"
        exit(1)
...
def Convert2FusionAlignment(sam_input, sam_normal_output, sam_fusion_output, log_file):
    print >> sys.stderr, "[%s] Convert to genome coordinate" % (right_now())
    
    if os.path.exists(sam_normal_output) and \
       os.path.exists(sam_fusion_output) and \
	   rerun_all == 0:
	return sam_normal_output
    
    syn_junc_cmd = [bin_dir + "Convert2FusionAlignment", sam_input, sam_normal_output, sam_fusion_output]
    ...
    try:    
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Convert to genome coordinate failed"
            exit(1)
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: Convert2FusionAlignment not found on this system"
        exit(1)
...
def main(argv=None):
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "hv:o:s:n:m:i:x:w:S:R:B:p:t:u:c:f:a:d:g:T:D:U:M:P:N:X:G:L:H:E:Y:I:Q:A:F:C:e:K:r:O:1:2:", 
                                        ["version",
                                         "help",  
                                         "seed-length=",
                                         "splice-mismatches=",
	...
	if len(args) == 1:
	    ...
	    if option == ("--d_range_fusion"):
		d_range_fusion = value;
	    if option == ("--std_region_extension"):
		std_region_extension = value;
	    if option == ("--filter-fusion-by-repeat"):
		do_filter_fusion_by_repeat = 1;
		chrom_blat = value;
	    if option == ("--synthetic_chromosome_dir"):
		synthetic_chromosome_dir = value;
	...
	if print_argu > 0:
	    ...
	    print >> argu_log, "std_region_extension=[%s]" % (std_region_extension)
	    
	    print >> argu_log, "d_range_fusion=[%s]" % (d_range_fusion)
	    
	    print >> argu_log, "synthetic_chromosome_dir=[%s]" % (synthetic_chromosome_dir)
	...
	std_range_out = output_dir + "std_range_file.txt";
	
	DNA2StdRegion(d_range_fusion, std_range_out, std_region_extension, logging_dir + "DNA2StdRegion.log");
	
	generate_combined_sequence(std_range_out, synthetic_chromosome_dir, chromosome_files_dir, 0, logging_dir + "generate_combined_sequence.log");
	
	separated_chromosome_dir = synthetic_chromosome_dir + "/splitchromosome/";
	
	bwt_idx_prefix = synthetic_chromosome_dir + "/bowtie_index";
	
	#check_bowtie_index(bwt_idx_prefix, all_chromosomes_file, 0, FASTA_file_extension)
	
	check_bowtie_index(bwt_idx_prefix, synthetic_chromosome_dir, 0, FASTA_file_extension)
	
	#check_split_index(split_index, chromosome_files_dir, 0, FASTA_file_extension, 5000000, 4, 5);
	
	real_all_chromos_path = read_dir_by_suffix(chromosome_files_dir, FASTA_file_extension);
	
	real_chromo_fai_file = temp_dir + "read_chromo.fai"
	
	real_chromo_size_file = read_chromo_sizes(real_all_chromos_path, 
	                                     temp_dir + "real_chrom_sizes", 
	                                     fusion_dir + "real_chrName.txt", 
	                                     temp_dir + "real_chrom_head",
	                                     real_chromo_fai_file);
	
	
	all_chromos_path = read_dir_by_suffix(separated_chromosome_dir, FASTA_file_extension);
	...
	if input_reads_2 != "":
		...
		print >> sys.stderr, "read files not equal in -1 and -2"
		exit(0)
	    ...
	    check_reads_format(combined_reads, format_flag, pairend);
	else:
	    check_reads_format(input_reads_1, format_flag, pairend);
	
	maxlen = extract_maxlen(logging_dir + "check_reads_format.log");
	
	read_width = maxlen;
	#####Run mapsplice multi-thread version

	maxlen = read_width;
	
	max_insertion = max_insert;
	
	max_deletion = min_intron_length;
	
	do_fusion_and_pairend = False;
		
	if junction_tobe_syn == "":
	    original_sam = call_mapsplice_multithreads(splice_mismatches, 
		                                       format_flag,
		                                       max_hits * 10,
		                                       bowtie_threads,
		                                       seg_len,
	    ...
	    for a in range(1, bowtie_threads + 1):
		combine_file = combine_file + [original_dir + "original.sam." + str(a)];
	    
	    cat_files(combine_file, original_dir + "original.sam");
	    
	    remove_files(combine_file);
	    
	    #####Convert to junction here
    
	    sam2juncarray([original_sam], original_dir + "ori.all_junctions.txt", separated_chromosome_dir, 
		     maxlen, 1, 350000000, "", 1, logging_dir + "sam2junc_ori.all_junctions.log")
	    
	    copy_file(original_dir + "ori.all_junctions.txt", output_dir + "ori.all_junctions.txt");
	    ...
	    combine_junc(junction_tobe_syn, temp_dir + "combined_junction.txt", logging_dir + "combined_junc.log");
	    
	    copy_file(temp_dir + "combined_junction.txt", output_dir + "combined_junction.txt");
	    
	    junction_tobe_syn = output_dir + "combined_junction.txt";
	...
	remapped_sam = call_mapsplice_multithreads(splice_mismatches, 
	                                           format_flag,
	                                           max_hits * 10,
	                                           bowtie_threads,
	                                           seg_len,
	...
	for a in range(1, bowtie_threads + 1):
	    combine_file = combine_file + [remap_dir + "remapped.sam." + str(a)];
	
	cat_files(combine_file, remap_dir + "remapped.sam");
	...
	for a in range(1, bowtie_threads + 1):
	    combine_unmapped_1 = combine_unmapped_1 + [remap_dir + "remap_unmapped." + str(a) + ".1"];
	
	cat_files(combine_unmapped_1, remap_dir + "remap_unmapped.1");
	
	remove_files(combine_unmapped_1);
	...
	if is_paired > 0:
	    ...
	    for a in range(1, bowtie_threads + 1):
		combine_unmapped_2 = combine_unmapped_2 + [remap_dir + "remap_unmapped." + str(a) + ".2"];
	    
	    cat_files(combine_unmapped_2, remap_dir + "remap_unmapped.2");
	    
	    remove_files(combine_unmapped_2);
	...
	#####Separate sam file and sort
	
	#original_sam_file = remapped_sam;
	
	#unspliced = original_dir + "original.unspliced";
	
	#spliced = original_dir + "original.spliced";
	
	#small_del = original_dir + "original.small_del";
	
	#small_ins = original_dir + "original.small_ins";
	
	#clipped = original_dir + "original.clipped";
	
	#unmapped = original_dir + "original.unmapped";
	
	#head = original_dir + "original.head";
	
	#SepSam(original_sam_file, 
	       #unspliced,
	       #spliced,
	       #small_del,
	       #small_ins,
	       ...
	       #unmapped,
	       #head,
	       #logging_dir + "SepOriginalSam.log");
	
	#original_separated_sams = [unspliced] + [spliced] + [small_del] + [small_ins] + [clipped];
	
	#original_mapped_sorted = sort_by_name_c(original_separated_sams, original_dir + "original_mapped_sorted.sam");
	...
	#remapped.sam_filtered_normal_alignments.bothunspliced.fusion_paired
	#remapped.sam_filtered_normal_alignments.fusion_paired
	...
	cat_files(combine_fusion_paired, \
	          fusion_paired_alignments);
	
	single_alignments = input_sam_file + "_filtered_normal_alignments" + ".single.comb";
	#remapped.sam_filtered_normal_alignments.bothunspliced.single
	#remapped.sam_filtered_normal_alignments.single
	...
	cat_files(combine_single, \
	          single_alignments);
	
	#paired_alignments = input_sam_file + "_filtered_normal_alignments" + ".paired.comb";
	#remapped.sam_filtered_normal_alignments.bothunspliced.paired
	#remapped.sam_filtered_normal_alignments.paired
	#combine_paired 
	...
	#cat_files(combine_paired, \
	          #paired_alignments);
	
	copy_file(filtered_alignment_base + ".ori.junc", output_dir + "ori.junc.txt");
	
	copy_file(filtered_alignment_base + ".ori.junc.del", output_dir + "ori.junc.deletion.txt");
	
	copy_file(filtered_alignment_base + ".ori.junc.ins", output_dir + "ori.junc.insertion.txt");
	
	#copy_file(input_sam_file, output_dir + "remapped.sam");
	...
	if do_fusion > 0:
	    ...
	    call_mapsplice_multithreads_fusion(splice_mismatches, 
	                                       format_flag,
	                                       max_hits * 10,
	                                       bowtie_threads,
	                                       seg_len,
	                                       ...
	                                       fusion_dir + "bowtie_original.output",
	                                       syn_idx_prefix,
	                                       "",
	                                       cluster_region,
	    ...
	    ####Add fusion alignments to fusion junctions and premium filtering here
	    ...
	    fusionsam2junc_filteranchor_newfmt(fusion_dir + "fusion_alignments_original.sam", 
	                                       ...
	                                       separated_chromosome_dir, logging_dir + "fusionsam2junc_original_fusion_junction.log")
	    
	    fusion_junction_bef_remap = fusion_dir + "original_fusion_junction.txt";
	    
	    ####Add fusion remapping here
	    
	    ####Build fusion index
	    
	    if no_original_fusion == False:
		
		fusion_junc_db(junction_tobe_syn, fusion_junction_bef_remap, separated_chromosome_dir, 2, 
		...
		call_mapsplice_multithreads_fusion(splice_mismatches, 
		                                   ...
		                                   fusion_dir + "bowtie.output",
		                                   syn_idx_prefix,
		                                   fusion_syn_idx_prefix,
		                                   cluster_region,
		                                   fusion_dir + "fusion_alignments.sam",
		                                   no_original_junction,
		                                   no_original_fusion,
		...
		if is_paired > 0:
		    comb_fusion_alignments = [single_alignments] + [fusion_paired_alignments] + [fusion_dir + "fusion_alignments.sam"];
		
		else:
		    comb_fusion_alignments = [fusion_dir + "fusion_alignments.sam"];
		    
		comb_fusion_alignments_sorted = sort_by_name_c(comb_fusion_alignments, fusion_dir + "combined_fusion_normal.sam");
		
		#combine fusio unmapped reads
		
		combine_fusion_unmapped_1 = [];
	
		for a in range(1, bowtie_threads + 1):
		    combine_fusion_unmapped_1 = combine_fusion_unmapped_1 + [fusion_dir + "fusion_unmapped." + str(a) + ".1"];
		
		cat_files(combine_fusion_unmapped_1, fusion_dir + "fusion_unmapped.1");
		
		remove_files(combine_fusion_unmapped_1);
		...
		if is_paired > 0:
		    combine_fusion_unmapped_2 = [];
		    
		    for a in range(1, bowtie_threads + 1):
			combine_fusion_unmapped_2 = combine_fusion_unmapped_2 + [fusion_dir + "fusion_unmapped." + str(a) + ".2"];
		    
		    cat_files(combine_fusion_unmapped_2, fusion_dir + "fusion_unmapped.2");
		    
		    remove_files(combine_fusion_unmapped_2);
	...
	add_head = final_alignments; #[temp_dir + "chrom_head"] + final_alignments;
	
	cat_files(add_head, temp_dir + "final_alignments_headed.sam");
	
	#convert to coordinate of genome
	
	Convert2FusionAlignment(temp_dir + "final_alignments_headed.sam", temp_dir + "final_alignments_headed.sam.normal", 
	                        temp_dir + "final_alignments_headed.sam.fusion", logging_dir + "Convert2FusionAlignment.log");
	
	
	#filter genome coordinate alignments
	
	####Filter alignments with fusion
		
	filtered_genome_coordinate_alignment_base = temp_dir + "_filtered_genome_coordinate_alignments";
	
	filtered_genome_coordinate_alignment_append = "_filtered_genome_coordinate_alignments";
	
	input_sam_file = temp_dir + "final_alignments_headed.sam.normal";
	...
	run_alignment_handler_multi("", 
                                    ...
                                    max_mate_dist,
                                    max_hits * 10,
                                    filtered_genome_coordinate_alignment_base,
                                    filtered_genome_coordinate_alignment_append,
                                    max_read_length,
                                    chromosome_files_dir,
                                    filter_flag,
                                    min_insertion,
                                    ...
                                    min_encompass_count,
                                    input_sam_file,
                                    logging_dir + "alignment_handler_genome_coordinate.log",
                                    logging_dir + "alignment_handler_genome_coordinate.err")
	

	genome_coordinate_alignments = input_sam_file + "_filtered_genome_coordinate_alignments"
	
	unmapped_reads_sams = unmapped_reads_sams + [temp_dir + "_filtered_genome_coordinate_alignments.unmapped"]
	
	genome_coordinate_paired_alignments = [input_sam_file + "_filtered_genome_coordinate_alignments" + ".paired"] \
	                                    + [input_sam_file + "_filtered_genome_coordinate_alignments" + ".bothunspliced.paired"];
	
	
	genome_coordinate_final_alignments = [];
	
	#if do_fusion > 0 and is_paired > 0:
	    #if no_original_fusion == False:
	genome_coordinate_final_alignments = [genome_coordinate_alignments] + genome_coordinate_paired_alignments;
	
	genome_coordinate_add_head = [temp_dir + "real_chrom_head"] + genome_coordinate_final_alignments;
	
	cat_files(genome_coordinate_add_head, temp_dir + "final_genome_coordinate_alignments_headed.sam");
	
	sam2bam(temp_dir + "final_genome_coordinate_alignments_headed.sam", output_dir + "alignments.bam", real_chromo_fai_file);
        
	copy_file(filtered_genome_coordinate_alignment_base + ".fil.junc", output_dir + "filtered_normal_junctions.txt");
	
	copy_file(filtered_genome_coordinate_alignment_base + ".fil.junc.del", output_dir + "filtered_normal_junctions_deletion.txt");
	
	copy_file(filtered_genome_coordinate_alignment_base + ".fil.junc.ins", output_dir + "filtered_normal_junctions_insertion.txt");
	
	copy_file(filtered_genome_coordinate_alignment_base + ".stat", output_dir + "normal.stat");
	
	#copy_file(filtered_genome_coordinate_alignment_base + ".stat", output_dir + "fusion.stat");
	
	copy_file(filtered_genome_coordinate_alignment_base + ".ori.junc.fusion.encompass.no.compass", output_dir + "fusion_junction.txt");
	
	#if do_fusion > 0:
	    #copy_file(filtered_fusion_alignment_base + ".stat", output_dir + "fusion.stat");
	    
	    #if is_paired > 0:
		#copy_file(filtered_fusion_alignment_base + ".ori.junc.fusion.encompass.no.compass", output_dir + "fusion_junction.txt");
	    #else:
		#copy_file(filtered_fusion_alignment_base + ".ori.junc.fusion.encompass.no.compass", output_dir + "fusion_junction.txt");
