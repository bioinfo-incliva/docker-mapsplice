...
def remove_files(files_tobe_remove):
    print >> sys.stderr, "[%s] remove files" % (right_now())
...
def copy_file(files_tobe_copy, copied_file):
    print >> sys.stderr, "[%s] copy file" % (right_now())
...
def read_chromo_sizes(all_sams_path, chromo_size_file, chrom_names, chrom_head):
    ...
    #merge_sam_cmd = merge_sam_cmd + [chrom_head] + [chromo_size_file] + [chrom_names]
...
def sam2bam(bowtie_mapped_sam, bam_file, allchromos_file):
...
def main(argv=None):
	...
	if input_reads_2 != "":
	    ...
	    check_reads_format(input_reads_1 + ',' + input_reads_2, format_flag, pairend);
	else:
	...
	if junction_tobe_syn == "":
	    ...
	    #filterjuncbyROCarguNoncanon(filteredbest_dir + "ori.all_junctions.filtered_by_min_mis_lpq.remained.txt", 
	...
	fusion_paired_alignments = input_sam_file + "_filtered_normal_alignments" + ".fusion_paired";
	
	single_alignments = input_sam_file + "_filtered_normal_alignments" + ".single";
	...
	sam2bam(temp_dir + "final_alignments_headed.sam", output_dir + "alignments.bam", all_chromosomes_file);
