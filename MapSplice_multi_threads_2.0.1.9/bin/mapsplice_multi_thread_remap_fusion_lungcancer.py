...
def searchunmappedreads(samfile, samfile_normal, reads_files, log_file):
    print >> sys.stderr, "[%s] search unmapped reads" % (right_now())
    
    mapsplice_log = open(log_file, "w")
   
    merge_sam_cmd = [bin_dir + "search_unmapped_reads"]  

    merge_sam_cmd = merge_sam_cmd + [samfile] + [samfile_normal];
    
    allexist = 0;
    
    shouldexist = 0;  
    
    samfile_normal = samfile + ".normal";
	...
	if os.path.exists(read_file + ".unmapped"):
	    allexist = allexist + 1;
	    

    if shouldexist == allexist and os.path.exists(samfile_normal):
	return samfile
    ...
    try:    
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: search unmapped reads failed"
            exit(1)
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: search_unmapped_reads not found on this system"
        exit(1)
...
def fusion_junc_db(junc_file, fusion_junc_file, chromosome_files_dir, min_anchor_width, 
    ...
    sed_log = open(fusion_dir + "fusion_replaced.txt", "w")
    ...
    fusion_junc_file = fusion_dir + "fusion_replaced.txt";
...
def main(argv=None):
	...
	if len(args) == 1:
	    ...
	    if option in ("-2", "--end_2"):
                input_reads_2 = value
	    if option == ("--original_fusion"):
		original_fusion = value;
	...
	if print_argu > 0:
	    ...
	    print >> argu_log, "original_fusion=[%s]" % (original_fusion)
	...
	splitted_reads_comb = splitted_reads_comb + splitted_reads1;
	
	if is_paired > 0:
	    splitted_reads_comb = splitted_reads_comb + splitted_reads2;
	
	sam_formatted_normal = sam_formatted + ".normal";
	
	searchunmappedreads(sam_formatted, sam_formatted, splitted_reads_comb, logging_dir + "unmapped_reads.log");
	
	remove_files(splitted_reads_comb);
	
	remove_files([sam_formatted]);
	    ...
	    #searchunmappedreads(sam_formatted, splitted_reads2, logging_dir + "unmapped_reads_2.log");
	    #combine_unmapped_2 = [];
	...
	input_sam_file = sam_formatted_normal; #sam_formatted + ".normal";
	...
	if do_fusion > 0:
	    ...
	    unmapped_fq1 = input_reads_1 + ".unmapped";
	    
	    unmapped_fq2 = "";
	    
	    if is_paired > 0:
		unmapped_fq2 = input_reads_2 + ".unmapped"; #remap_dir + "remap_unmapped.2";
	    ...
	    fusion_junction_bef_remap = original_fusion; #fusion_dir + "original_fusion_junction.txt";
		...
		fusion_junc_db(junction_tobe_syn, fusion_junction_bef_remap, chromosome_files_dir, 2, 
		               21, 20, 50, fusion_dir + "fusion_synthetic_sequence.txt", logging_dir + "fusion_junc_db.log");
	...
	copy_file(filtered_alignment_base + ".fil.junc", output_dir + "filtered_normal_junctions.txt");
	
	copy_file(filtered_alignment_base + ".fil.junc.del", output_dir + "filtered_normal_junctions_deletion.txt");
	
	copy_file(filtered_alignment_base + ".fil.junc.ins", output_dir + "filtered_normal_junctions_insertion.txt");
	
	copy_file(filtered_alignment_base + ".stat", output_dir + "normal.stat");
