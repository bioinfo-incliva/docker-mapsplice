...
def gtf2exon(gtf_file,
             gtf_exons_file,
             log_file):
    
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Convert gene gtf file to sorted exon file" % start_time.strftime("%c")
    
    if os.path.exists(gtf_exons_file) and \
	   rerun_all == 0:
	return gtf_exons_file

    ##############convert gene gtf file to annotation format#################
    
    gtf2genetab_log = open(log_file, "w");
...
def sepMPSfusion(infusion,
                 min_intron,
                 log_file):
    
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Separate MPS fusion to inter_chromosome long_range inversion deletion duplication with max intron %s " % (start_time.strftime("%c"), str(min_intron))
    
    if os.path.exists(infusion + ".interchr") and \
       os.path.exists(infusion + ".longrange") and \
       os.path.exists(infusion + ".inversion") and \
       os.path.exists(infusion + ".del_dup") and \
	   rerun_all == 0:
	return infusion
    ...
    gtf2genetab_cmd = [bin_dir + "sepMPSfusion",
                       infusion,
                       str(min_intron)]
    ...
    try:
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Separate fusion to inter_chromosome long_range inversion deletion duplication failed"
            exit(1)
...
def sepdRangerfusion(infusion,
                     min_intron,
                     log_file):
    
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Separate dRanger fusion to inter_chromosome long_range inversion deletion duplication with max intron %s " % (start_time.strftime("%c"), str(min_intron))
    
    if os.path.exists(infusion + ".interchr") and \
       os.path.exists(infusion + ".longrange") and \
       os.path.exists(infusion + ".inversion") and \
       os.path.exists(infusion + ".del") and \
       os.path.exists(infusion + ".dup") and \
	   rerun_all == 0:
	return infusion
    ...
    gtf2genetab_cmd = [bin_dir + "sepdRangerfusion",
                       infusion,
                       str(min_intron)]
    ...
    try:
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Separate dRanger fusion to inter_chromosome long_range inversion deletion duplication failed"
            exit(1)
           
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: sepdRangerfusion not found on this system"
        exit(1)

def swapMPS_and_dRanger_matched(MPS_matched_dRanger,
                                MPS_matched_dRanger_swapped,
                                log_file):
	       
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Swap dRanger MPS matched fusion " % (start_time.strftime("%c"))
    
    if os.path.exists(MPS_matched_dRanger_swapped) and \
	   rerun_all == 0:
	return MPS_matched_dRanger_swapped

    gtf2genetab_log = open(log_file, "w");
    
    gtf2genetab_cmd = [bin_dir + "swap_dRanger_and_MPS_matched",
                       MPS_matched_dRanger,
                       MPS_matched_dRanger_swapped]
    ...
    try:
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Swap dRanger MPS matched fusion failed"
            exit(1)
           
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: swap_dRanger_and_MPS_matched not found on this system"
        exit(1)
	
def comp_fusiondb_offset(MPS_fusion,
                         dRanger_fusion,
                         matched_fusion,
                         match_range,
                         sampleid,
    ...
    print >> sys.stderr, "[%s] Compare MapSplice fusion to dRanger fusion with match range %s " % (start_time.strftime("%c"), str(match_range))
    
    if os.path.exists(matched_fusion) and \
	   rerun_all == 0:
    ...
    gtf2genetab_cmd = [bin_dir + "comp_fusiondb_offset",
                       MPS_fusion,
                       dRanger_fusion,
                       matched_fusion,
                       str(match_range),
    ...
    try:
        ...
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Compare MapSplice fusion to dRanger fusion failed"
            exit(1)
           
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: comp_fusiondb_offset not found on this system"
        exit(1)
	
def annotate_fusion(in_fusion,
                    annotated,
                    gene_exon_file,
                    log_file):
    ...
    print >> sys.stderr, "[%s] Annotate fusion junction by sorted exons" % start_time.strftime("%c")
    
    #splice_cmd = ""
    
    #filtered_alignment_file = input_sam_file + filtered_alignment_append;
    
    if os.path.exists(annotated) and \
	   rerun_all == 0:
	return annotated
 	
    ##############annotate fusion junctions##################
    
    annotate_fusion_log = open(log_file, "w");
...
def grepexcludefusion(annotated,
               longintron_intergenic_fusion):
    
    print >> sys.stderr, "[%s] extract long intron and inter chromosomes fusion from annotated fusion " % start_time.strftime("%c")
    
    if os.path.exists(longintron_intergenic_fusion) and \
	   rerun_all == 0:
	return longintron_intergenic_fusion
 	
    ############## exclude fusion #################
    
    print >> sys.stderr, "[%s] exclude normal fusion which both ends falls in same gene" % start_time.strftime("%c")
    
    exclude_normal_log = open(temp_dir + "exclude_normal_fusions.txt", "w");
    
    exclude_normal_cmd = ["grep",
                           "-v", "normal",
                           annotated]
    
    if DEBUG == 1:
	print >> sys.stderr, "[%s]" % exclude_normal_cmd
    
    try:
        retcode = subprocess.call(exclude_normal_cmd, stdout=exclude_normal_log)     
	
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: grep normal failed"
            exit(1)
           
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: grep not found on this system"
        exit(1)	
	
    ############## exclude fusion #################
    
    print >> sys.stderr, "[%s] exclude overlap fusion which both ends falls in overlapped gene?" % start_time.strftime("%c")
    
    exclude_overlapped_log = open(longintron_intergenic_fusion, "w");
    
    exclude_overlapped_cmd = ["grep",
                              "-v", "overlapping",
                              temp_dir + "exclude_normal_fusions.txt"]
    
    if DEBUG == 1:
	print >> sys.stderr, "[%s]" % exclude_overlapped_cmd
    
    try:
        retcode = subprocess.call(exclude_overlapped_cmd, stdout=exclude_overlapped_log)     
	
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: grep overlapping failed"
            exit(1)
...
def grepexcludestrfusion(infusion,
                         excludestr,
                         excluded_fusion):
    print >> sys.stderr, "[%s] grep exclude %s fusion from annotated fusion " % (right_now(), excludestr)
    
    if os.path.exists(excluded_fusion) and \
	   rerun_all == 0:
	return excluded_fusion
 	
    ############## exclude fusion #################

    exclude_normal_log = open(excluded_fusion, "w");
    ...
    try:
        ...
        if retcode != 0 and retcode != 1:
            print >> sys.stderr, fail_str, "Error: grep exclude failed"
            exit(1)
...
def grepincludestrfusion(infusion,
                         excludestr,
                         excluded_fusion):
    print >> sys.stderr, "[%s] grep include %s fusion from annotated fusion " % (right_now(), excludestr)
    ...
    try:
        ...
        if retcode != 0 and retcode != 1:
            print >> sys.stderr, fail_str, "Error: grep include failed"
            exit(1)
...
def ConvertFusionAlignments(infusion,
                            extension,
                            inalignmentfile,
                            out_converted_alignments,
                            genegtffile,
                            gene_converted_gtf_file,
                            in_chromosome_dir,
                            synthetic_chromosomes,
                            fusion_profile,
                            out_converted_alignments_fusion,
                            log_file):
    
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Convert fusion alignments in genome coordinate to synthetic fusion gene chromosome coordinate " % (start_time.strftime("%c"))
    
    if os.path.exists(out_converted_alignments) and \
       os.path.exists(gene_converted_gtf_file) and \
       os.path.exists(synthetic_chromosomes) and \
       os.path.exists(fusion_profile) and \
       os.path.exists(out_converted_alignments_fusion) and \
	   rerun_all == 0:
	return out_converted_alignments
 	
    log_handler = open(log_file, "w");
    
    include_str_cmd = [bin_dir + "generate_fusiongene_convert_coordinate_trim",
                       infusion,
                       str(extension),
                       inalignmentfile,
                       out_converted_alignments,
    ...
    if DEBUG == 1:
	print >> sys.stderr, "[%s]" % include_str_cmd
    
    try:
        retcode = subprocess.call(include_str_cmd, stdout=log_handler)     
	
        if retcode != 0:
            print >> sys.stderr, fail_str, "Error: Convert fusion alignments in genome coordinate to synthetic fusion gene chromosome coordinate  failed"
            exit(1)
           
    except OSError, o:
        if o.errno == errno.ENOTDIR or o.errno == errno.ENOENT:
            print >> sys.stderr, fail_str, "Error: generate_fusiongene_convert_coordinate_trim not found on this system"
        exit(1)


def ConvertFusionAlignmentsDNA(infusion,
                               extension,
                               inalignmentfile,
                               out_converted_alignments,
                               genegtffile,
    ...
    print >> sys.stderr, "[%s] Convert DNA matched fusion alignments in genome coordinate to synthetic fusion gene chromosome coordinate " % (start_time.strftime("%c"))
    ...
    include_str_cmd = [bin_dir + "generate_fusiongene_convert_coordinate_trim_dRanger",
                       infusion,
                       str(extension),
                       inalignmentfile,
                       out_converted_alignments,
...
def addstrandconsistent(annnoated_fusion,
                        annnoated_stranded_fusion,
                        log_file):
    
    start_time = datetime.now()
    
    print >> sys.stderr, "[%s] Add gene strand consistent to annotated fusion" % start_time.strftime("%c")

    ##############add fusion strand consistent##################
    
    add_fusion_strand_consistent_log = open(log_file + ".log", "w");
    
    add_fusion_strand_consistent_err = open(log_file + ".err", "w");
...
def sortbam(bam_file, bam_sorted_file):
    print >> sys.stderr, "[%s] sort bam file" % (right_now())

    if os.path.exists(bam_sorted_file) and \
	   rerun_all == 0:
    ...
    sam2import_cmd = [bin_dir + "samtools", "sort", bam_file, bam_sorted_file]
	...
	if retcode != 0:
	    print >> sys.stderr, fail_str, "Error: sort sam to bam failed"
	    exit(1)
...
def indexbam(bam_sorted_file):
    print >> sys.stderr, "[%s] index bam file" % (right_now())


    sam2pileup_cmd = ""

    sam2import_cmd = [bin_dir + "samtools", "index", bam_sorted_file]
	...
	if retcode != 0:
	    print >> sys.stderr, fail_str, "Error: index sam to bam failed"
	    exit(1)
...
def main(argv=None):
	...
	if len(opts) == 0:
	    ...
	    if option in ("-l", "--seglen"):
                seg_len = int(value)
	    if option in ("-O"):
                fusions_to_be_visualized = value
		...
		gene_gtf_file = value;
	    if option == "--Ext":
		visualize_extension = int(value);
	    if option == "--Long-range-intron":
		min_intron = int(value);
	    if option == "--DNA-match-range":
		match_range = int(value);
	    if option == "--DNA-fusion":
		dRanger_fusion = (value);		
		...
		if value == "phred64":
		    qual_scale = value;
		    qual_scale_set_by_user = qual_scale;
		elif value == "phred33":
		    qual_scale = value;
		    qual_scale_set_by_user = qual_scale;
		elif value == "solexa64":
	...
	if print_argu > 0:
        ...
        print >> sys.stderr
        print >> sys.stderr, "[%s] Beginning conversion for visualization (v%s)" % (right_now(), get_version())
        print >> sys.stderr, "-----------------------------------------------" 
	...
	gtf2exon(gene_gtf_file,
	         temp_dir + "gene_exons.txt",
	         logging_dir + "gtf2exon.log");
	
	
	################add comparison to DNA fusion##############################
	
	if dRanger_fusion != "":
	    
	    MPS_fusion_copied = fusion_dir + "MPS_fusion_tobe_visualized";
	    
	    copy_file(fusions_to_be_visualized, MPS_fusion_copied);	    
	    
	    sepMPSfusion(MPS_fusion_copied,
	                 min_intron,
	                 logging_dir + "sepMPSfusion.log");
	    
	    MPS_long_rang_interchr_fusion = [];
	    
	    MPS_long_rang_interchr_fusion = MPS_long_rang_interchr_fusion + [MPS_fusion_copied + ".longrange"];
	    
	    MPS_long_rang_interchr_fusion = MPS_long_rang_interchr_fusion + [MPS_fusion_copied + ".interchr"];
	    
	    cat_files(MPS_long_rang_interchr_fusion, MPS_fusion_copied + ".longrange.interchr");
	    
	    ########################################
	    
	    dRanger_fusion_copied = fusion_dir + "dRanger_fusion_tobe_visualized";
	    
	    copy_file(dRanger_fusion, dRanger_fusion_copied);
	    
	    sepdRangerfusion(dRanger_fusion_copied,
	                     min_intron,
	                     logging_dir + "sepdRangerfusion.log");
	    
	    dRanger_long_rang_interchr_fusion = [];
	    
	    dRanger_long_rang_interchr_fusion = dRanger_long_rang_interchr_fusion + [dRanger_fusion_copied + ".header"];
			
	    dRanger_long_rang_interchr_fusion = dRanger_long_rang_interchr_fusion + [dRanger_fusion_copied + ".longrange"];
	    
	    dRanger_long_rang_interchr_fusion = dRanger_long_rang_interchr_fusion + [dRanger_fusion_copied + ".interchr"];
	    
	    cat_files(dRanger_long_rang_interchr_fusion, dRanger_fusion_copied + ".longrange.interchr");
	    
	    comp_fusiondb_offset(MPS_fusion_copied + ".longrange.interchr",
	                         dRanger_fusion_copied + ".longrange.interchr",
	                         fusion_dir + "MPS_VS_dRanger_fusion.matched",
	                         match_range,
	                         dRanger_fusion,
	                         dRanger_fusion,
	                         logging_dir + "comp_fusiondb_offset.log");
	    
	    swapMPS_and_dRanger_matched(fusion_dir + "MPS_VS_dRanger_fusion.matched",
	                                output_dir + "MPS_VS_dRanger_fusion.matched.swapped",
	                                logging_dir + "swapMPS_and_dRanger_matched.log");
	    
	    fusions_to_be_visualized = output_dir + "MPS_VS_dRanger_fusion.matched.swapped"
	
	##########################################################################
	
	annotate_fusion(fusions_to_be_visualized,
	                fusion_dir + "fusions_annotated.txt",
	                temp_dir + "gene_exons.txt",
	                logging_dir + "annotate_fusion.log");

	#grepexcludefusion(fusion_dir + "fusions_annotated.txt",
	                  #fusion_dir + "fusions_annotated_longintron_intergenic.txt");
	
	fusions_selected = fusion_dir + "fusions_annotated.txt";
	
	###########################Optional##################################################
	
	if dRanger_fusion == "":
	    print >> sys.stderr, "[%s] extract long intron and inter chromosomes fusion from annotated fusion " % (right_now())
	    
	    grepexcludestrfusion(fusion_dir + "fusions_annotated.txt",
		                 "normal",
		                 fusion_dir + "fusions_annotated_exclude_normal.txt");	
	    
	    grepexcludestrfusion(fusion_dir + "fusions_annotated_exclude_normal.txt",
		                 "overlapping",
		                 fusion_dir + "fusions_annotated_exclude_normal_exclude_overlap.txt");	
    
	    addstrandconsistent(fusion_dir + "fusions_annotated_exclude_normal_exclude_overlap.txt",
		                fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt",
		                logging_dir + "addstrandconsistent");
	    
	    grepincludestrfusion(fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.notmatch",
		                 "intergenic",
		                 fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.notmatch.intergenic");
	    
	    fusion_to_be_cat = [];
	    
	    fusion_to_be_cat = fusion_to_be_cat + [fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.match"];
	    
	    fusion_to_be_cat = fusion_to_be_cat + [fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.notmatch.intergenic"];
	    
	    cat_files(fusion_to_be_cat, fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.match.plus.intergenic");
	    
	    fusions_selected = fusion_dir + "fusions_annotated_longintron_intergenic_stranded.txt.match.plus.intergenic";
	
	#########Convert alignmetn with selected annotated fusion##################################
	
	if dRanger_fusion == "":
	    ConvertFusionAlignments(fusions_selected,
		                    visualize_extension,
		                    sam_formatted,
		                    fusion_dir + "alignments_converted.sam",
		                    gene_gtf_file,
		                    temp_dir + "gene_converted.gtf",
		                    chromosome_files_dir,
		                    output_dir + "synthetic_genome.fa",
		                    output_dir + "fusion_gene_profile.txt",
		                    fusion_dir + "alignments_converted_fusion.sam",
		                    logging_dir + "ConvertFusionAlignments.log");
	else:
	    ConvertFusionAlignmentsDNA(fusions_selected,
	                               visualize_extension,
	                               sam_formatted,
	                               fusion_dir + "alignments_converted.sam",
	                               gene_gtf_file,
	...
	copy_file(temp_dir + "gene_converted.gtf", output_dir + "gene_converted.gtf");
	
	######all alignments to bam#######################################################
	
	sam2bam(fusion_dir + "alignments_converted.sam", temp_dir + "alignments_converted.bam", chromo_fai_file);
	
	sortbam(temp_dir + "alignments_converted.bam", output_dir + "alignments_converted_sorted");
		
	indexbam(output_dir + "alignments_converted_sorted.bam");
	
	######fusion alignments to bam#######################################################
	
	sam2bam(fusion_dir + "alignments_converted_fusion.sam", temp_dir + "alignments_converted_fusion.bam", chromo_fai_file);
		
	sortbam(temp_dir + "alignments_converted_fusion.bam", output_dir + "alignments_converted_fusion_sorted");
		
	indexbam(output_dir + "alignments_converted_fusion_sorted.bam");	
	...
	print >> sys.stderr
        print >> sys.stderr, "[%s] Finishing conversion for visualization (time used: %s)" % (right_now(), duration)
        print >> sys.stderr, "-----------------------------------------------" 
