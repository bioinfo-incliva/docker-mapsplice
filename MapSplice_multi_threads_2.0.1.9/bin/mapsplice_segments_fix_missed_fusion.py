...
def juncdb_bwt2sam_new(bowtie_mapped, misinfo, sam_formatted, log_file):
    print >> sys.stderr, "[%s] Converting juncdb bowtie mapped to SAM format" % (right_now())
    ...
    #bowtie2sam_cmd = ["bowtie2sam", 
                        #bowtie_mapped,
                        #sam_formatted]  
    bowtie2sam_cmd = [bin_dir + "juncdb_bwt2sam_new", 
                        bowtie_mapped,
                        sam_formatted,
			str(misinfo)] 
...
def sam2fq(sam_formatted, converted_read_file, flag, log_file):
    ...
    if os.path.exists(converted_read_file) and \
	       rerun_all == 0:
...
def main(argv=None):
	...
	if print_argu > 0:
        ...
        print >> sys.stderr
        print >> sys.stderr, "[%s] Beginning remap unmapped reads to missed junction run (v%s)" % (right_now(), get_version())
        print >> sys.stderr, "-----------------------------------------------" 
	...
	chromo_fai_file = temp_dir + "chrom.fai";
	...
	junc_db(junction_tobe_syn, chromosome_files_dir, min_anchor_length, 
                maxlen - min_anchor_length, 50, remap_dir + "synthetic_alljunc_sequenc.txt",
	...
	#-----------------convert to sam file---------------------#
	juncdb_bwt2sam_new(syn_bwt_map, 0, remap_dir + "syn_remapped.sam", logging_dir + "syn_remapped.log")
	
	syn_remapped_sorted = remap_dir + "syn_remapped.sam";
	    
	files_tobe_cat = [temp_dir + "chrom_head"] + [remap_dir + "syn_remapped.sam"];
	    
	cat_files(files_tobe_cat, output_dir + "alignments.sam");
	
	sam2bam(output_dir + "alignments.sam", temp_dir + "alignments.bam", chromo_fai_file);
	
	sortbam(temp_dir + "alignments.bam", output_dir + "alignments_sorted");
	
	indexbam(output_dir + "alignments_sorted.bam");
