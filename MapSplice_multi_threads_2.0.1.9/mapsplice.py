...
def collectstats(final_stats, 
                 ...
                 unmapped_reads_sams,                 
                 log_file):
    print >> sys.stderr, "[%s] Collect stats of reads alignments and junctions" % (right_now())
    ...
    merge_sam_cmd = merge_sam_cmd + [str(is_paired)] + [str(do_fusion)] + [str(do_repeat)] + [str(do_annotate)];
    ...
    for sam_file in unmapped_reads_sams:
	merge_sam_cmd = merge_sam_cmd + [sam_file]
...
def SetUnmappedBitFlag(unmappedsamfile, unmappedsetbit, log_file):
    print >> sys.stderr, "[%s] Set unmapped paired end reads bit flag" % (right_now())
    
    if os.path.exists(unmappedsetbit):
...
def main(argv=None):
	...
	if input_reads_2 != "":
	    ...
	    check_reads_format(combined_reads, min_read_len, pairend);
	else:
	    check_reads_format(input_reads_1, min_read_len, pairend);
	
	qual_scale_detected = ""
	
	(maxlen, format_flag, qual_scale_detected) = extract_maxlen(logging_dir + "check_reads_format.log");
	
	if qual_scale_set_by_user == "":
	...
	if do_fusion > 0:
	    ...
	    filteroriginalfusion(fusion_dir + "original_fusion_junction.txt", 
