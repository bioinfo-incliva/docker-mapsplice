...
static void parseOptions(int argc, const char **argv) {
	...
	do {
		...
		switch (next_option) {
			...
			case ARG_MIN_MAPPED: min_mapped_len = parseInt(1, "--min_map_len minimum mapped read length must be at least 1"); break;
                   ...
                   const string& outfile)
{
	...
	for(int i = 0; i < nthreads; i++)
	{
		...
		ofstream* new_output_fs = new ofstream(mapsplice_output_file_threaded.c_str());
		if(*new_output_fs)
	...
	if(output_unmapped)
	{
		for(int i = 0; i < nthreads; i++)
		{
			string mapsplice_unmapped_file1_threaded = output_unmapped_filename + "." + int_to_str(i + 1) + ".1";
			string mapsplice_unmapped_file2_threaded = output_unmapped_filename + "." + int_to_str(i + 1) + ".2";
