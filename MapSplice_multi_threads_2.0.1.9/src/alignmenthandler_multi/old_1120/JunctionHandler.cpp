...
bool JunctionHandler::Init(const vector<string>& alignment_files, size_t max_read_width, int insert_len, int delete_len, 
	const string& chrom_dir, size_t min_anchor, size_t min_mismatch, size_t min_junc_anchor, size_t min_fusion_coverage)
{
	Clear();
...
void JunctionHandler::SamRec2Junc(SamRec& samrec, size_t sam_count, hash_map<string, hash_map<size_t, JuncSeedSp> >& cur_read_junc)
{
	...
	if (spliceway_vec2.size() > 1)
	{
		...
		size_t flankidx2 = samrec.flankstrings.size() - 1;
	...
	if (samrec.is_fusion)
	{
		...
		string ins_str = samrec.flankstrings[flankidx];;

		prefixst = samrec.fusion_prefix_st;

		prefixend = samrec.fusion_prefix_end;

		prefixlen = spliceway_vec.back().second; //samrec.fusion_prefix_len;//spliceway_vec.back().second;
		...
		suffixlen = spliceway_vec2.front().second; //samrec.fusion_suffix_len;//spliceway_vec2.front().second;
		...
		if ((*sam_rec_iter)->encompassed_juncs.size())
		{
			...
			for (; juncseed_iter != juncseed_end; ++juncseed_iter)
			{
				...
				if (juncseed_iter->second)
				{
					...
					((juncseed_iter->first))->m_fusion_encompassing_reads_doner.push_back(samrec);//(*sam_rec_iter)->tostring(0, 0));
				...
				else
				{
					...
					((juncseed_iter->first))->m_fusion_encompassing_reads_acceptor.push_back(samrec);//(*sam_rec_iter)->tostring(0, 0));
	...
	for (chrom_iter = m_junc_sort.begin(); chrom_iter != m_junc_sort.end(); ++chrom_iter)
	{
		...
		for (junc_sort_iter = junc_sort_vec.begin(); junc_sort_iter != junc_sort_vec.end(); ++junc_sort_iter)
		{
			if ((*junc_sort_iter)->m_hits == 0)
				(*junc_sort_iter)->m_filtered_type = FILTERED_BY_FUSION_SMALL_ANCHOR;
...
int JunctionHandler::FindFragmentLengthReverse(vector<vector<int> >& paths, vector<pair<size_t, size_t> >& exons, size_t end_point, vector<size_t>& stored_fragment_lengths)
{
	...
	for (size_t j = 0; j < paths.size(); ++j)
	{
		...
		if (k != cur_path.size())
		{
			...
			stored_fragment_lengths.push_back(cur_fragment_length);
		}
	}
...
int JunctionHandler::FindFragmentLengthForward(vector<vector<int> >& paths, vector<pair<size_t, size_t> >& exons, size_t end_point, vector<size_t>& stored_fragment_lengths)
{
...
int JunctionHandler::PrintExonExpression(vector<vector<int> >& paths, vector<pair<size_t, size_t> >& exons, vector<vector<int> >& exon_express, ofstream* ofs_ptr)
{
...
void JunctionHandler::WriteFusionJunctionWithEncompassingAlignments(string junction_fusion_file, JunctionHandler* filtered_junction_ptr)
{
	...
	string fragment_file = "fusion_fragments.txt";
	...
	for (chrom_iter = m_junc_sort.begin(); chrom_iter != m_junc_sort.end(); ++chrom_iter)
	{
		...
		for (junc_sort_iter = junc_sort_vec.begin(); junc_sort_iter != junc_sort_vec.end(); ++junc_sort_iter)
		{
			if ((*junc_sort_iter)->m_is_fusion)
			{
				...
				{
					if ((*junc_sort_iter)->m_fusion_encompassing_reads_doner[i].tag_base_name != (*junc_sort_iter)->m_fusion_encompassing_reads_acceptor[i].tag_base_name)
						cout << "warning: tag base name not consistent"<<endl<<(*junc_sort_iter)->m_fusion_encompassing_reads_doner[i].tostring(0, 0) 
						<< (*junc_sort_iter)->m_fusion_encompassing_reads_acceptor[i].tostring(0, 0)<<endl;
					//cout << "is in region doner"<<endl;

					bool is_in_region = (*junc_sort_iter)->m_fusion_encompassing_reads_doner[i].is_in_region(left_arrived_exons);

					if (is_in_region)
					{
						//cout << "is in region acceptor"<<endl;
						is_in_region = (*junc_sort_iter)->m_fusion_encompassing_reads_acceptor[i].is_in_region(right_arrived_exons);
					}
					...
					if (is_in_region)
					{
						...
						fusion_encompassing_reads_doner.push_back(&((*junc_sort_iter)->m_fusion_encompassing_reads_doner[i]));

						fusion_encompassing_reads_acceptor.push_back(&((*junc_sort_iter)->m_fusion_encompassing_reads_acceptor[i]));
					}
					else
					{
						...
						ofs_fusion << (*junc_sort_iter)->m_fusion_encompassing_reads_doner[i].tostring(0, 0) <<endl;

						ofs_fusion << (*junc_sort_iter)->m_fusion_encompassing_reads_acceptor[i].tostring(0, 0) <<endl;
				...
				for (size_t i = 0; i < fusion_encompassing_reads_doner.size(); ++i)
				{
					...
					for (size_t j = 0; j < doner_fragments.size(); ++j)
					{
						...
						{
							combined_fragments.push_back(doner_fragments[j] + acceptor_fragments[k]);

							fusion_fragments.push_back(doner_fragments[j] + acceptor_fragments[k]);

							fusion_encompassing_reads_doner[i]->fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);

							fusion_encompassing_reads_acceptor[i]->fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);

							ofs_fusion << doner_fragments[j] + acceptor_fragments[k] << endl;

							cout << "fragment length:"<< doner_fragments[j] + acceptor_fragments[k] << endl;

							if (doner_fragments[j] + acceptor_fragments[k] < fragment_length)
				...
				for (size_t i = 0; i < (*junc_sort_iter)->doner_side_spanning_pairs.size(); ++i)
				{
					...
					for (size_t j = 0; j < doner_fragments.size(); ++j)
					{
						for (size_t k = 0; k < acceptor_fragments.size(); ++k)
						{
							...
							(*junc_sort_iter)->doner_side_spanning_pairs[i].first.fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);

							(*junc_sort_iter)->doner_side_spanning_pairs[i].second.fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);
				...
				for (size_t i = 0; i < (*junc_sort_iter)->accetpr_side_spanning_pairs.size(); ++i)
				{
					...
					for (size_t j = 0; j < doner_fragments.size(); ++j)
					{
						for (size_t k = 0; k < acceptor_fragments.size(); ++k)
						{
							...
							(*junc_sort_iter)->accetpr_side_spanning_pairs[i].first.fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);

							(*junc_sort_iter)->accetpr_side_spanning_pairs[i].second.fragment_lengths.push_back(doner_fragments[j] + acceptor_fragments[k]);
				...
				;
				////print exon expression
				PrintExonExpression((*junc_sort_iter)->left_paths, (*junc_sort_iter)->left_exons, left_exon_express, &ofs_fusion);

				PrintExonExpression((*junc_sort_iter)->right_paths, (*junc_sort_iter)->right_exons, right_exon_express, &ofs_fusion);
				...
				ofs_fusion_no_compass << cur_junc.to_normal_junction(fusion_count)<<'\t';//<<endl;
...
//			}
			...
			//	offset_iter->second.m_filtered_type = FILTERED_BY_NONCAN_LEFT_RIGHT_PAIR;
			//}
			else if (!isindel && offset_iter->second.m_flankcase > 4 && (offset_iter->second.m_entropy < 2.5) && (offset_iter->second.m_left_paired_count == 0 || offset_iter->second.m_right_paired_count == 0) && paired)
			{
