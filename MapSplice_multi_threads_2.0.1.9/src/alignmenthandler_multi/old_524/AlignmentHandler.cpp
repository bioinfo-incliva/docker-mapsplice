	...
	for (sam_rec_iter = read_sam.begin(); sam_rec_iter != read_sam.end(); ++sam_rec_iter)
	{
		...
		if ((*sam_rec_iter)->is_fusion)
		{
			...
			else 
			{
				...
				suffixlen = (*sam_rec_iter)->fusion_prefix_len;//spliceway_vec.back().second;
		...
		else
		{
			...
			for (oft_mpl_iter = (*sam_rec_iter)->spliceway_vec.begin(); oft_mpl_iter != (*sam_rec_iter)->spliceway_vec.end() - 1; ++oft_mpl_iter)
			{
				...
				if ((*sam_rec_iter)->tag_name.find("FAM157A:1243:1292") != string::npos && ((*sam_rec_iter)->start == 197907696) || (*sam_rec_iter)->start == 238058)
				{				
					cout << "check filter score"<<endl;

					cout << (*sam_rec_iter)->tostring(0, 0) << endl;

					cout <<"m_entropy:"<< junc_hash_comb_iter->second.m_entropy << endl;

					cout <<"m_lpq:"<< junc_hash_comb_iter->second.m_lpq << endl;
...
void* AlignmentHandler::ProcessPEReadMulti(void * str)
{
	while (true)
	{
		...
		if (paired == false)
		{
			...
			if (/*cur_same_read_alignments_paired.first.size()*/ isend1mapped && isend2mapped/*cur_same_read_alignments_paired.second.size()*/)
			{
			...
			{
				if (isend1mapped/*cur_same_read_alignments_paired.first.size()*/)
				{
				...
				else if (isend2mapped/*cur_same_read_alignments_paired.second.size()*/)
				{
...
void* AlignmentHandler::ProcessPEReadFilterJuncMulti(void * str)
{
	while (true)
	{
		...
		//cout << "test mate unmapped"<<endl;

		//cout << "paired:"<< paired << endl;

		//cout << "cur_same_read_alignments_paired.first.size():"<< cur_same_read_alignments_paired.first.size() << endl;

		//cout << "cur_same_read_alignments_paired.second.size():"<< cur_same_read_alignments_paired.second.size() << endl;
...
void AlignmentHandler::ProcessFile(string alignment_file, JunctionHandler* junc_handler, JunctionHandler* junc_handler_filtered, int fileter_by_junc)
{
	...
	else
	{
		...
		else
		{
			...
			//string unmapped_str = alignment_file; unmapped_str.append(basename2(m_filtered_alignment_file)); unmapped_str.append(".unmapped");
...
AlignmentHandler::FilterAlignment()
{
	...
	string unmapped_str = m_filtered_alignment_file; /*unmapped_str.append(basename2(m_filtered_alignment_file));*/ unmapped_str.append(".unmapped");
	...
	//m_junction_handler.Load2SimpJunc(m_original_junc_locs);
	...
	if (m_do_filter & 4)
	{
		...
		//FilterFusionByIsolatedExons(&m_junction_handler);
	...
	//	cerr << "estimate fragment length: " << endl;

	//	t1=clock();

	//	m1 = time(NULL);

	//	EstimateFragmentLength(&m_junction_handler);
	...
	//	cerr << "estimate fragment length finished: " << m_any_time << endl;
	...
	//cout << "generate structure: " << m_any_time << endl;
...
AlignmentHandler::WriteAlignment(vector<SamRec*>& sam_rec_ptr)
{
	...
	for (sam_rec_ptr_iter = sam_rec_ptr.begin(); sam_rec_ptr_iter != sam_rec_ptr.end(); ++sam_rec_ptr_iter)
	{
		...
		else if ((*sam_rec_ptr_iter)->paired_type == NORMAL_PAIRED)
		{
			...
			//if ((*sam_rec_ptr_iter)->is_fusion == false && (*sam_rec_ptr_iter)->mate_diff > 0)
			//{
			//	//if (140009089 == (*sam_rec_ptr_iter)->end || (*sam_rec_ptr_iter)->mate_offset == 140008951)
			//	//	cerr << (*sam_rec_ptr_iter)->tostring(0, 0) << endl<<(*sam_rec_ptr_iter)->end << endl;

			//	m_normal_paired_locs[(*sam_rec_ptr_iter)->chrom_name].push_back(make_pair((*sam_rec_ptr_iter)->end, (*sam_rec_ptr_iter)->mate_offset));
			//}
			//cerr << "paired"<<endl;
		}
		else
		...
		if ((m_do_filter & 128) == 0)
		{
			for (sam_rec_ptr_iter = pair_samvec_iter->first.begin(); sam_rec_ptr_iter != pair_samvec_iter->first.end() && is_unspliced; ++sam_rec_ptr_iter)
			{
				...
				else if ((*sam_rec_ptr_iter)->paired_type == NORMAL_PAIRED)
				{
					...
					//if ((*sam_rec_ptr_iter)->mate_diff > 0)
					//{
					//	m_normal_paired_locs[(*sam_rec_ptr_iter)->chrom_name].push_back(make_pair((*sam_rec_ptr_iter)->end, (*sam_rec_ptr_iter)->mate_offset));
					//}
					//cerr << "paired"<<endl;
...
AlignmentHandler::FilterPairedSamRecStatic(void * str)
{
	...
	if (vps_iter->paired_sam_rec.first->is_fusion)
	{
		...
		//if (vps_iter->paired_sam_rec.first->tag_base_name.find("HWUSI-EAS1531_0001:4:105:1863:93#0") != string::npos)
		//{
		//	cout << vps_iter->paired_sam_rec.first->tostring(0, 0)<<endl;

		//	cout << vps_iter->paired_sam_rec.second->tostring(0, 0)<<endl;
		//}
...
bool AlignmentHandler::EstablishPairingStatic(void * str)
{
	...
	//cout << "FilterPairedSamRecStatic" << endl;
		...
		if (chrom_iter->first.find("~") != string::npos)
		{
			...
			for (offset_iter = chrom_iter->second.begin(); offset_iter != chrom_iter->second.end(); ++offset_iter)
			{
				if (offset_iter->second.m_is_fusion && offset_iter->second.m_filtered_type == NOT_FILTERED)
				{
					...
					//if (find_start_region && find_end_region && ((string(chr1) != string(chr2)) || (start_region_idx != end_region_idx)))
					//{
					//	
					//}
					//else if (start_region_idx != end_region_idx)
...
{
	//junc_handler->LoadUnfilteredJuncToSortVec();
	...
	for (chrom_iter = m_normal_paired_locs.begin(); chrom_iter != m_normal_paired_locs.end(); ++chrom_iter)
	{
		...
		if (m_original_junc_locs.find(chrom_iter->first) != m_original_junc_locs.end())
		{
			...
			for (normal_pair_iter = cur_normal_pairs.begin(); normal_pair_iter != cur_normal_pairs.end(); ++normal_pair_iter)
			{
				...
				//if (lower_bound_iter != cur_sorted_juncs.end())
				//	cout << (*lower_bound_iter)->to_normal_junction(0)<<endl;

				vector<pair<int, int> >::iterator junction_iter = lower_bound_iter;

				//if (junction_iter != cur_sorted_juncs.begin())
				//	--junction_iter;
...
AlignmentHandler::FindFusionJuncRegionStatic(void * str)
{
	...
	while (true)
	{
		...
		else if ( ((cur_fusion_region_iter->m_doner_st <= cur_fusion_region.m_doner_st && cur_fusion_region_iter->m_doner_end >= cur_fusion_region.m_doner_st) ||
			       (cur_fusion_region_iter->m_doner_end >= cur_fusion_region.m_doner_end && cur_fusion_region_iter->m_doner_st <= cur_fusion_region.m_doner_end)) && 
				  ((cur_fusion_region_iter->m_acceptor_st <= cur_fusion_region.m_acceptor_st && cur_fusion_region_iter->m_acceptor_end >= cur_fusion_region.m_acceptor_st) ||
				   (cur_fusion_region_iter->m_acceptor_end >= cur_fusion_region.m_acceptor_end && cur_fusion_region_iter->m_acceptor_st <= cur_fusion_region.m_acceptor_end)))
		{
			++regions_count;
...
void* AlignmentHandler::FilterSingleMultiStatic(void * str)
{
	...
	if (sam_rec_ptr.front()->tag_name.find("FAM157A:1243:1292") != string::npos)
	{
		cout << "FAM157A:1243:1292:size1:"<< sam_rec_ptr.size()<<endl;
	}
	...
	else 
	{
		...
		if (sam_rec_ptr.front()->tag_name.find("FAM157A:1243:1292") != string::npos)
		{
			cout << "FAM157A:1243:1292:size3:"<< sam_rec_ptr.size()<<endl;
		}
		...
		//	if ((*sam_rec_ptr.begin())->filter_score != (*(sam_rec_ptr.begin() + 1))->filter_score)
		//	{
		//		cout << "filter_score not equal"<<endl;

		//		//cout.unsetf(ios::floatfield);

		//		//cout.precision(20);

		//		cout << (*(sam_rec_ptr.begin() + 1))->filter_score <<endl;

		//		cout <<  (*sam_rec_ptr.begin())->filter_score <<endl;

		//		cout << (*(sam_rec_ptr.begin() + 1))->filter_score - (*sam_rec_ptr.begin())->filter_score <<endl;
		//	}
		//	for (sam_rec_ptr_iter = sam_rec_ptr.begin(); sam_rec_ptr_iter != sam_rec_ptr.end(); ++sam_rec_ptr_iter)
		//	{
		//		cout << (*sam_rec_ptr_iter)->tostring(0, 0) << endl;

		//		cout << "filter_score:"<<(*(sam_rec_ptr_iter))->filter_score<<endl;

		//		cout << "mappedlen:"<< (*(sam_rec_ptr_iter))->mappedlen<<endl;

		//		cout << "ave_junc_mis:"<< (*(sam_rec_ptr_iter))->ave_junc_mis<<endl;

		//		cout << "m_contig_len:"<<(*(sam_rec_ptr_iter))->m_contig_len<<endl;

		//		cout << "isexonic:"<< (*(sam_rec_ptr_iter))->isexonic<<endl;

		//		cout << "ave_intron_len:"<< (*(sam_rec_ptr_iter))->ave_intron_len<<endl;
		//	}
		//}
		...
		else
		{
			...
			{
				if ((*(same_vps_iter - 1))->mappedlen == (*same_vps_iter)->mappedlen && !comp_filter_score_range((*(same_vps_iter - 1))->filter_score, (*same_vps_iter)->filter_score)//(vps_iter - 1)->mate_dist == (vps_iter)->mate_dist 
					//&& (vps_iter - 1)->total_pairing_rate == (vps_iter)->total_pairing_rate 
					//&& (vps_iter - 1)->total_anchor_len == (vps_iter)->total_anchor_len
					/*&& (same_vps_iter - 1)->total_ave_mismatch == (same_vps_iter)->total_ave_mismatch*/)
				{
		...
		for (sam_rec_ptr_iter = sam_rec_ptr.begin() + 1; sam_rec_ptr_iter != sam_rec_ptr.end(); ++sam_rec_ptr_iter)
		{
			if ((*sam_rec_ptr_iter)->tag_name.find("FAM157A:1243:1292") != string::npos)
			{
				cout << (*(sam_rec_ptr_iter - 1))->tostring(0, 0) << endl << (*sam_rec_ptr_iter)->tostring(0, 0) << endl;

				cout << "filter_score:"<<endl<< (*(sam_rec_ptr_iter - 1))->filter_score << endl<< (*(sam_rec_ptr_iter))->filter_score<<endl;

				cout << "mappedlen:"<<endl<< (*(sam_rec_ptr_iter - 1))->mappedlen << endl<< (*(sam_rec_ptr_iter))->mappedlen<<endl;

				cout << "ave_junc_mis:"<<endl<< (*(sam_rec_ptr_iter - 1))->ave_junc_mis << endl<< (*(sam_rec_ptr_iter))->ave_junc_mis<<endl;

				cout << "m_contig_len:"<<endl<< (*(sam_rec_ptr_iter - 1))->m_contig_len << endl<< (*(sam_rec_ptr_iter))->m_contig_len<<endl;

				cout << "isexonic:"<<endl<< (*(sam_rec_ptr_iter - 1))->isexonic << endl<< (*(sam_rec_ptr_iter))->isexonic<<endl;

				cout << "ave_intron_len:"<<endl<< (*(sam_rec_ptr_iter - 1))->ave_intron_len << endl<< (*(sam_rec_ptr_iter))->ave_intron_len<<endl;
			}
	...
	if (sam_rec_ptr.front()->tag_name.find("FAM157A:1243:1292") != string::npos)
	{
		cout << "FAM157A:1243:1292:size2:"<< sam_rec_ptr.size()<<endl;
	...
	if (sam_rec_ptr.size() > m_max_hits)
	{
		...
		if (sam_rec_ptr.front()->tag_name.find("HWI-EAS217:4:55:1764:913#0/1") != string::npos)
		{
			cout << "set unmapped in filter single multi"<<endl;

			cout << sam_rec_ptr.front()->tostring(0, 0) << endl;
		}
	}
