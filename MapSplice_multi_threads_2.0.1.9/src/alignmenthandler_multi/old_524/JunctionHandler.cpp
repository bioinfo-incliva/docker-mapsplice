				...
				for (size_t i = 0; i < fusion_encompassing_reads_doner.size(); ++i)
				{
					...
					for (size_t j = 0; j < doner_fragments.size(); ++j)
					{
						for (size_t k = 0; k < acceptor_fragments.size(); ++k)
						{
							...
							ofs_fusion <<"fragment length:"<< doner_fragments[j] + acceptor_fragments[k] << endl;
							...
							if (doner_fragments[j] + acceptor_fragments[k] < fragment_length)
							{
								...
								{
									if (doner_fragment_counts.find(doner_fragments[j]) == doner_fragment_counts.end())
										doner_fragment_counts[doner_fragments[j]] = 1;
									else
										++doner_fragment_counts[doner_fragments[j]];
								}
								...
								cur_fusion_fragments.push_back(doner_fragments[j] + acceptor_fragments[k]);

								if (doner_fragments[j] > max_doner_fragment)
									max_doner_fragment = doner_fragments[j];

								if (acceptor_fragments[k] > max_acceptor_fragment)
									max_acceptor_fragment = acceptor_fragments[k];
							}
						}
					}	
