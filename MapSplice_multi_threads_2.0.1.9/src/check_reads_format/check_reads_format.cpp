...
int main(int argc, char* argv[])
{
	...
	if (firstline[0] == '>')
	{
		...
		cerr << argv[1] << " is fasta format"<<endl;
	}
	else if (firstline[0] == '@')
	{
		fasta = 0;
		...
		cerr << argv[1] << " is fastq format"<<endl;
	}
	else
	{
		cerr << "unknown read format"<<endl;
	...
	cerr <<"total "<< countall << " reads"<<endl;

	cout <<"maxlen "<<maxlen<<" minlen "<<minlen<<endl;

	cerr <<"maxlen "<<maxlen<<" minlen "<<minlen<<endl;

	if (!fasta)
	{
		cout <<"maxqualscore "<<maxqual<<" minqualscore "<<minqual<<endl;

		cerr <<"maxqualscore "<<maxqual<<" minqualscore "<<minqual<<endl;
	}

	if (minqual > 64 && maxqual < 126)
	{
		cerr << "quality scacle looks like is Phred+64"<<endl;

		cout << "qual_scale phred64"<< endl;

	}
	else if (minqual > 59 && maxqual < 126)
	{
		cerr << "quality scacle looks like is Solexa+64"<<endl;

		cout << "qual_scale solexa64"<< endl;
	}
	else if (minqual > 33 && maxqual < 126)
	{
		cerr << "quality scacle looks like is Phred+33"<<endl;

		cout << "qual_scale phred33"<< endl;
	}
	else
	{
		cerr << "unknown quality score"<<endl;
	}
