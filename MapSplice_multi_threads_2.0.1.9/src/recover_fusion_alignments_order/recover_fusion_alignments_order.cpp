...
{
	return lhs.zfzfusstr < lhs.zfzfusstr;
...
void extract_and_convert_sam(char* fusion_alignment_file, char* ordered_fusion_alignment_file)
{
	...
	while(readnextalignmenthandler.ReadNextTagAlignPE(stored_tag_aligns_pe, cur_read_id))
	{
		...
		{
			cerr << "multi alignments"<<endl;

			for (size_t i = 0; i < merged_tag_aligns_pe.first.size(); ++i)
			{
				cerr << merged_tag_aligns_pe.first[i]->tostring(merged_tag_aligns_pe.first.size(), i+1) << endl;
			}

			for (size_t i = 0; i < merged_tag_aligns_pe.second.size(); ++i)
			{
				cerr << merged_tag_aligns_pe.second[i]->tostring(merged_tag_aligns_pe.second.size(), i+1) << endl;
			}
...
int main(int argc, char** argv)
{
	...
	extract_and_convert_sam(fusion_alignment_file, converted_fusion_alignment_file);
}